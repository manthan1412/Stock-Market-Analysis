...
class historicData:
    ...
    def download_symbol(self, symbol):
        ...
        fileName = "{0}-{1}-{2}-{3}123.csv".format(symbol, self.timeFrame, self.startDate, self.endDate)
        exists = os.path.isfile(fileName)
        exists = False
        if exists == False:
            message = "HIT,{0},{1},{2},{3},5,093000,160000,1\n".format(symbol, self.timeFrame, self.startDate, self.endDate)
...
import subprocess
import time
is_running = subprocess.Popen('pgrep iqfeed && echo Running', shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
(out, err) = is_running.communicate()
if err.decode('ascii'):
    print(err.decode('ascii'))
elif 'Running' not in out.decode('ascii'):
    proc = subprocess.Popen('sudo /etc/init.d/iqfeed start', shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    (out, err) = proc.communicate()
    if out:
        print('Output of subprocess:', out)
    if err:
        print('Error occured starting iqfeed:', err)
    else:
        time.sleep(4)
else:
    print("Process already running")
dateStart = datetime.datetime(2018,1,1)
dateEnd = datetime.datetime(2018,1,5)
symbolOne = "AAPL"
iq = historicData(dateStart, dateEnd, 300)
symbolOneData = iq.download_symbol(symbolOne)
if True:
    print("Just checking")
